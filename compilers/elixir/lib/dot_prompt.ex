defmodule DotPrompt do
    ...
    try do
      ...
      case Parser.parse(tokens) do
        ...
        {:ok, ast} ->
          ...
          vary_vars = extract_vary_vars(ast.body)
          ...
          # Debugging schema_params full map
          Logger.debug(
            "DotPrompt.do_parse_schema params for #{prompt_name}: #{inspect(schema_params)}"
          )
      ...
      :ok ->
        ...
        case handle_response_contracts(ast.body, warnings) do
          ...
          {:ok, warnings, first_schema} ->
            init_vars = extract_init_vars(init)
  ...
  def render(prompt_name_or_content, params, runtime, opts \\ []) do
    ...
    case compile(prompt_name_or_content, params, opts) do
      {:ok, %DotPrompt.Result{} = compile_result} ->
        defaults = extract_defaults_from_metadata(compile_result.metadata)
  ...
  defp expand_fragment_by_path(
    ...
    if is_collection?(path, context.current_dir) do
      ...
      spec = Map.get(context.fragment_defs, path) || %{}
  ...
  defp extract_vary_vars(nodes) when is_list(nodes) do
  ...
  defp extract_vary_vars(_), do: MapSet.new()
  ...
  defp extract_init_vars(init) do
    # Scan fragment definitions for @var patterns (e.g. match: @skill_names)
    vars =
    ...
    # Also scan param defaults if they are strings containing @var (rare but possible)
  ...
  defp extract_defaults_from_metadata(%{params: params}) when is_map(params) do
    params
    |> Enum.filter(fn {_name, spec} -> Map.has_key?(spec, :default) and spec.default != nil end)
    |> Enum.into(%{}, fn {name, spec} ->
      clean_atom = String.to_atom(name)
      {clean_atom, spec.default}
    end)
  end

  defp extract_defaults_from_metadata(_), do: %{}
end
