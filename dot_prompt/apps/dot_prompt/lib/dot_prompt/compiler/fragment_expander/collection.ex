defmodule DotPrompt.Compiler.FragmentExpander.Collection do
  ...
  @spec expand(String.t(), map(), integer(), map(), integer(), map()) ::
          {:ok, iodata(), map(), integer()} | {:error, String.t()}
  def expand(collection_path, params, indent \\ 0, acc_files \\ %{}, acc_count \\ 0, rules \\ %{}) do
    ...
    if File.dir?(full_path) do
      ...
      if matching_files == [] and match_rule != nil do
        ...
        {:ok, [header_text], Map.merge(acc_files, dir_meta), acc_count + 1}
      else
        ...
        Enum.reduce_while(
          results,
          {[], Map.merge(acc_files, dir_meta), acc_count},
          fn
            {:ok, {:ok, content, item_files, name_only}}, {t_acc, f_acc, c_acc} ->
              section = [
                "\n[[section:frag:",
                to_string(indent),
                ":",
              ...
              {:cont, {[t_acc, section], Map.merge(f_acc, item_files), c_acc + 1}}
        ...
        |> case do
          {:error, _} = err -> err
          {full_iodata, merged_meta, final_count} -> {:ok, full_iodata, merged_meta, final_count}
        end
      end
    else
